...
import os
import json
from config import sentiment_batch_size, sentiment_lr, poem_corpus_dir, roberta_chinese_model_path, poem_head_corpus_path
...
def read_corpus(dir_path):
    ...
    读原始数据
    """
    datas = np.load(poem_head_corpus_path)
    data = datas['data']
    ix2word = datas['ix2word'].item()
    word2ix = datas['word2ix'].item()

    data_poem = [[ix2word[ix] for ix in line if ix!=8292 and ix != 8290 and ix!=8291] for line in data]

    data = []
    for poem in data_poem:
        temp = []
        for word in poem:
    ...
    for poem in data:
        poem = "".join(poem)
        encode_poem = tokenizer.encode(poem)[0]
        if word2idx["[UNK]"] in encode_poem:
            # 过滤掉UNK字符
            continue
        if len(poem) == 5:
            sents_src.append(poem[0] + "##" + "五言")
            sents_tgt.append(poem)
            continue
        if len(poem) == 7:
            sents_src.append(poem[0] + "##" + "七言")
            sents_tgt.append(poem)
            continue
